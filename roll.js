...
import {
  ...
  consumablePacks,
  coupons,
} from "./entityData/consumablelist.js";
...
export class Roll {
  ...
  Shop(count = 3) {
    ...
    let upPool = this.game.upgradeDedupe
      ? upgradesList.filter(
          (up) => !this.game.upgrades.some((u) => u._name === up.name),
        )
      : upgradesList;

    let conPool = this.game.upgradeDedupe
      ? consumableList.filter(
          (c) => !this.game.consumables.some((pc) => pc._name === c.name),
        )
      : consumableList;
    ...
    return this._drawFromPool(fullPool, count, (entry) => {
      ...
      if (entry.type === "Upgrade") {
        ...
        item = new Tarot(entry);
      } else {
        item = new Consumable(entry);
      }

      if (item && item.price && this.game && this.game.discount) {
        item.price = Math.floor(
          Math.max(0, item.price * (1 - this.game.discount)),
        );
      }
      return item;
    });
  ...
  Stickers(up) {
    ...
    let available = stickers.filter(
      (sticker, index) => index < Settings.DIFFICULTY.id - 1,
    );
    let result = [];
  ...
  Vouchers(count = 1) {
    ...
    for (let n = 0; n < count; n++) {
      const item = this.weightedPick(
        available,
        this.game.voucherRand.bind(this),
      );
  ...
  ConsumablePacks(count = 2) {
    ...
    for (let n = 0; n < count; n++) {
      const item = this.weightedPick(
        consumablePacks,
        this.game.boosterRand.bind(this),
      );
